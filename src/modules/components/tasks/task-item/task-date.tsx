...
export const TaskDate: React.FC<TaskDateProps> = props => {
  ...
  if (momentDate.isSame(moment(), 'day') && momentDate.isBefore(moment()))
    return (
      <span className='text-xs text-red-400'>
        {momentDate.format('HH:mm:ss') == '23:59:59' ? 'hoy' : 'hoy, ' + momentDate.format('HH:mm')}
      </span>
    )
  ...
  if (momentDate.isSame(moment().subtract(1, 'day'), 'day'))
    return (
      <span className='text-xs text-red-400'>
        {momentDate.format('HH:mm:ss') == '23:59:59' ? 'ayer' : 'ayer, ' + momentDate.format('HH:mm')}
      </span>
    )
  ...
  if (momentDate.isBefore(moment(), 'day'))
    return (
      <span className='text-xs text-red-400'>
        {momentDate.format('HH:mm:ss') == '23:59:59'
          ? momentDate.format('DD/MM/YY')
          : momentDate.format('DD/MM/YY, HH:mm')}
      </span>
    )
  ...
  if (momentDate.isSame(moment().add(1, 'day'), 'day'))
    return (
      <span className='text-xs text-gray-400'>
        {momentDate.format('HH:mm:ss') == '23:59:59' ? 'mañana' : 'mañana, ' + momentDate.format('HH:mm')}
      </span>
    )
