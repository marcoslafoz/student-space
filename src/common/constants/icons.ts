// Logo
import StudentSpaceIcon from '/src/assets/images/svg/studentspace.svg'

export { StudentSpaceIcon }

// Sidebar
import DashboardIcon from '/src/assets/images/svg/dashboard.svg'
import CourseIcon from '/src/assets/images/svg/teacher.svg'
import SubjectIcon from '/src/assets/images/svg/book.svg'
import TaskIcon from '/src/assets/images/svg/task-square.svg'
import DocumentsIcon from '/src/assets/images/svg/folder-open.svg'
import EventsIcon from '/src/assets/images/svg/calendar.svg'
import SettingsIcon from '/src/assets/images/svg/settings.svg'
import MoonIcon from '/src/assets/images/svg/moon.svg'

export { DashboardIcon, CourseIcon, SubjectIcon, TaskIcon, DocumentsIcon, EventsIcon, SettingsIcon, MoonIcon }

// Others
import CloudSuccessIcon from '/src/assets/images/svg/cloud-success.svg'
import CloudErrorIcon from '/src/assets/images/svg/cloud-error.svg'
import ArrowLeftIcon from '/src/assets/images/svg/arrow-left.svg'
import PlusIcon from '/src/assets/images/svg/add-circle.svg'
import EditIcon from '/src/assets/images/svg/edit.svg'
import TrashIcon from '/src/assets/images/svg/trash.svg'

export { CloudSuccessIcon, CloudErrorIcon, ArrowLeftIcon, PlusIcon, EditIcon, TrashIcon }
