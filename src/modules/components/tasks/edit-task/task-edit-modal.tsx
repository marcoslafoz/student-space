...
import { Modal, ModalContent, ModalHeader, ModalBody, Input, ModalFooter, Button, Textarea, DatePicker, TimeInput } from '@nextui-org/react'
import { Task } from '../../../../common/types'
import { getLocalTimeZone, today } from '@internationalized/date'
import { SubmitHandler, useForm } from 'react-hook-form'
import { formatDate, formatLocalTimezoneToString, formatStringToLocalTimezone, formatTime } from '../../../../common/utils'
import moment from 'moment-timezone'
import { useLazyMutationEditTask, useLazyMutationRemoveTask, useLazyMutationRemoveTaskSubject } from '../../../../common/api/graphql/mutation'
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <div className='grid grid-cols-2 gap-5'>
              <DatePicker onChange={(e) => setValue('date', e.toString())} size='sm' label="Fecha" minValue={today(getLocalTimeZone())} defaultValue={formatDate(data.date)} />
              <TimeInput onChange={(e) => setValue('time', e.toString())} size='sm' label="Hora" hourCycle={24} defaultValue={formatTime(data.date)} />
            </div>
          ...
          <ModalFooter>
            <Button color='danger' className='bg-transparent border border-red-500 text-red-500' size='sm' onClick={handleRemoveTask}>Eliminar</Button>
            <Button color='primary' size='sm' type='submit' onClick={handleSubmit(onSubmit)} >Guardar</Button>
          </ModalFooter>
